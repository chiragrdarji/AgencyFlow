  ...
  app.post("/api/contact", async (req, res) => {
    try {
      ...
      const userMailOptions = {
        from: process.env.SMTP_FROM || "",
        to: data.email,
        subject: "Thank you for contacting SmartSync.One",
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #2563eb;">Thank You for Your Message</h2>
            <p>Hi ${data.firstname + data.name},</p>
            <p>Thank you for reaching out to SmartSync.One. We've received your message and will get back to you within 24 hours.</p>
            ...
            <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0;">
              ...
              <p><strong>Subject:</strong> ${data?.subject || "N/A"}</p>
              <p><strong>Company:</strong> ${data?.company || "N/A"}</p>
              <p style="background: #ffffff; padding: 15px; border-left: 4px solid #2563eb; margin: 10px 0;">${data.message || "N/A"
          }</p>
            </div>
